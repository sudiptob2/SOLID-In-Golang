...
// SOLVE : There is no exact solution to this problem
// Here we can say square does not have any width and height
// It only has a side
// then square struct will be : type struct square {side int}
// And we can implement a function when we need rectangle representation from the square
//
...
func (sq2 *square2) rectangle() *rectangle {
	return &rectangle{sq2.side, sq2.side}
}
...
func main() {
	...
	fmt.Println("using the square2")
	sq2 := square2{5}
	// we have to pass quadrangle subtype
	// sq2 is not a subtype of quadrangle
	useQuadrangle(sq2.rectangle())
	// So we can see the result are consistent now
